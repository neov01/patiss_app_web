...
export default async function DashboardPage({ searchParams }: { searchParams: DashboardSearchParams }) {
    ...
    // 1. Fetch real user
    const { data: adminProfile } = await supabase.from('profiles').select('*, organizations(*)').eq('id', user.id).single()
    ...
    if (kioskUserId && adminProfile.role_slug !== 'vendeur' && adminProfile.role_slug !== 'patissier') {
        const { data: kioskProfile } = await supabase.from('profiles').select('*, organizations(*)').eq('id', kioskUserId).single()
