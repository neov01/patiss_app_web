-- ============================================================
-- Migration : 20260811000000_ia_financial_context_kiosk_actor_fallback
-- Objectif  : Permettre à Croustik (assistant IA) de répondre depuis une
--             session Kiosque. Le mode Kiosque authentifie via un jeton
--             signé maison (pas une vraie session Supabase Auth) : le
--             serveur utilise alors un client service_role, sur lequel
--             auth.uid() renvoie toujours NULL. get_ia_financial_context
--             échouait donc systématiquement avec "Not authenticated" pour
--             tout utilisateur en mode Kiosque, quel que soit son rôle.
--
--             On reprend le mécanisme déjà utilisé par soft_delete_order_atomic /
--             restore_order_atomic : l'identité de l'acteur est passée
--             explicitement en paramètre et n'est utilisée que lorsqu'il n'y a
--             pas de vrai jeton (COALESCE(auth.uid(), p_actor_id)) ; si un vrai
--             jeton existe, il doit obligatoirement correspondre à l'acteur
--             déclaré, pour empêcher toute usurpation depuis une session normale.
--             Les helpers is_super_admin()/get_user_organization_id() étant
--             câblés sur auth.uid(), ils sont remplacés par une lecture directe
--             du profil de v_actor_id.
-- ============================================================

DROP FUNCTION IF EXISTS public.get_ia_financial_context(UUID);
