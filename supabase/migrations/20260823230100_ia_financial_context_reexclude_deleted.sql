-- ============================================================
-- Migration : 20260823230100_ia_financial_context_reexclude_deleted
-- Objectif  : Deux problèmes cumulés sur le contexte financier de Croustik.
--
--             1) Régression. La migration 20260809110004 avait exclu les
--                commandes supprimées logiquement de tous les agrégats
--                « orders ». La migration 20260811000000 (fallback acteur
--                Kiosque) a recréé la fonction à partir d'une version
--                antérieure et a donc perdu la totalité de ces filtres :
--                l'IA voyait de nouveau les commandes supprimées dans
--                commandes_impayees, commandes_a_venir, commandes_historique,
--                repartition_reception, repartition_canaux, top_clients et
--                kpis_globaux.
--
--             2) Les agrégats « transactions » n'ont jamais été filtrés.
--                Avec transactions.deleted_at (migration 20260823230000),
--                l'argent d'une commande supprimée sort maintenant aussi du
--                CA mensuel, du CA quotidien, des transactions récentes, de
--                la ventilation par moyen de paiement, des top produits et
--                des KPI globaux.
--
--             Le corps est repris intégralement de 20260811000000 (même
--             signature, même contrôle d'acteur Kiosque), seuls les filtres
--             sont ajoutés.
-- ============================================================

CREATE OR REPLACE FUNCTION public.get_ia_financial_context(
  p_org_id   UUID,
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL AND p_actor_id IS DISTINCT FROM v_actor_id THEN
    RAISE EXCEPTION 'Acteur invalide'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT (
    ...
    OR (v_actor_org_id IS NOT NULL AND v_actor_org_id = p_org_id)
  ) THEN
      ...
      FROM (
        SELECT
          ...
          (SELECT COUNT(*) FROM orders
           WHERE organization_id = p_org_id
             AND status NOT IN ('cancelled')
             AND deleted_at IS NULL) AS nb_commandes_total,
          (SELECT ROUND(AVG(total_amount)::numeric, 0) FROM orders
           WHERE organization_id = p_org_id
             AND status NOT IN ('cancelled')
             AND deleted_at IS NULL) AS panier_moyen_global
        FROM transactions
        WHERE organization_id = p_org_id
          AND deleted_at IS NULL
      ) k
...
REVOKE EXECUTE ON FUNCTION public.get_ia_financial_context(UUID, UUID) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_ia_financial_context(UUID, UUID) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_ia_financial_context(UUID, UUID) TO authenticated;
