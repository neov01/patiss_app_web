-- ============================================================
-- Migration : 20260809120000_soft_delete_restore_kiosk_actor_fallback
-- Objectif  : Permettre la suppression/restauration de commande depuis une
...
--             auth.uid() renvoie toujours NULL. Les deux fonctions
--             échouaient donc systématiquement avec "Non authentifié" pour
--             tout utilisateur en mode Kiosque, quel que soit son rôle.
--
--             On reprend le mécanisme déjà utilisé par encaisser_atomic :
--             l'identité de l'acteur est passée explicitement en paramètre
--             et n'est utilisée que lorsqu'il n'y a pas de vrai jeton
--             (COALESCE(auth.uid(), p_actor_id)) ; si un vrai jeton existe,
--             il doit obligatoirement correspondre à l'acteur déclaré,
--             pour empêcher toute usurpation depuis une session normale.
-- ============================================================

DROP FUNCTION IF EXISTS public.soft_delete_order_atomic(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS public.restore_order_atomic(UUID, UUID, TEXT);
...
REVOKE EXECUTE ON FUNCTION public.soft_delete_order_atomic(UUID, UUID, TEXT, UUID) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.soft_delete_order_atomic(UUID, UUID, TEXT, UUID) TO authenticated;

REVOKE EXECUTE ON FUNCTION public.restore_order_atomic(UUID, UUID, TEXT, UUID) FROM PUBLIC, anon, authenticated;
