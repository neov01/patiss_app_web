-- Migration : 20260614000000_fix_kiosk_atomic_rpc_auth.sql
-- Objectif  : Permettre l'exécution des RPC d'encaissement et de création de commande en mode Kiosque (via service_role) en fallbackant de manière sécurisée sur le créateur transmis.
...
AS $$
DECLARE
  -- Si auth.uid() est NULL (ex: clé service_role en mode Kiosque), on fallback de manière sécurisée sur p_created_by.
  -- Si auth.uid() est défini (utilisateur connecté standard), il est impossible d'usurper une autre identité.
...
AS $$
DECLARE
  -- Si auth.uid() est NULL (ex: clé service_role en mode Kiosque), on fallback de manière sécurisée sur created_by transmis dans le payload.
